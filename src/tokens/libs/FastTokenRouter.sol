...
abstract contract FastTokenRouter is TokenRouter {
     ...
     */
    function _transferTo(address _recipient, uint256 _amount, uint32 _origin, bytes calldata _metadata)
        internal
        virtual
    {
        address _tokenRecipient = _getTokenRecipient(_recipient, _amount, _origin, _metadata);
    ...
    function fillFastTransfer(
        ...
        bytes32 filledFastTransfersKey = _getFastTransfersKey(_origin, _fastTransferId, _amount, _fastFee, _recipient);
     ...
     */
    function fastTransferRemote(uint32 _destination, bytes32 _recipient, uint256 _amountOrId, uint256 _fastFee)
        public
        payable
        virtual
        returns (bytes32 messageId)
    {
        ...
        bytes memory metadata = _fastTransferFromSender(_amountOrId, _fastFee, _fastTransferId);

        messageId = _GasRouter_dispatch(
            _destination, msg.value, TokenMessage.format(_recipient, _amountOrId, metadata), address(hook)
        );
     ...
     */
    function _fastTransferFromSender(uint256 _amount, uint256 _fastFee, uint256 _fastTransferId)
        internal
        virtual
     ...
     */
    function _getTokenRecipient(address _recipient, uint256 _amount, uint32 _origin, bytes calldata _metadata)
        internal
        view
        returns (address)
    {
        ...
        (uint256 _fastFee, uint256 _fastTransferId) = abi.decode(_metadata, (uint256, uint256));

        address _fillerAddress =
            filledFastTransfers[_getFastTransfersKey(_origin, _fastTransferId, _amount, _fastFee, _recipient)];
    ...
    function _getFastTransfersKey(
        ...
        return keccak256(abi.encodePacked(_origin, _fastTransferId, _amount, _fastFee, _recipient));
    }
     ...
     */
    function _fastTransferTo(address _recipient, uint256 _amount) internal virtual;

    /**
     * @dev Should collect `amount` of tokens from `_sender`.
     * @dev The implementation is delegated.
     */
    function _fastRecieveFrom(address _sender, uint256 _amount) internal virtual;
}
